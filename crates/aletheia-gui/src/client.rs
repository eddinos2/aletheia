...
impl Client {
    ...
    pub fn rename(&self, session: &str, va: u64, name: &str) -> Result<(), String> {
        let params = format!(
            r#"{{"session":"{session}","va":"0x{va:x}","name":"{}"}}"#,
            escape_json(name)
        );
        let _ = self.call("rename", &params)?;
        Ok(())
    }
    ...
    pub fn xrefs(&self, session: &str, va: u64) -> Result<XrefsInfo, String> {
        ...
        Ok(XrefsInfo {
            from: extract_array_blob(&r, "\"from\""),
            to: extract_array_blob(&r, "\"to\""),
...
fn parse_functions(r: &str) -> Vec<FuncRow> {
    ...
    let mut out = Vec::new();
    // Split on `},{` boundaries inside objects — fragile but serde-free.
    let mut depth = 0i32;
